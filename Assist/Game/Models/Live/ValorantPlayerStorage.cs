...
public class ValorantPlayerStorage
{
    ...
    private async Task GetPlayername()
    {
        try
        {
            var t = await AssistApplication.Current.CurrentUser.DisplayNameService.FetchPlayersV2(PlayerId);
    ...
    private async Task GetPlayerMMR()
    {
        try
        {
            var t = await AssistApplication.Current.CurrentUser.Player.GetPlayerMmr(PlayerId);
            _playerMmr = t;
        }
        catch (RequestException e)
        {
